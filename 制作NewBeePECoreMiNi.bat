...
if %errorlevel%==1 set filelist=newbee&&set winver=win10&&echo 你选择了1
if %errorlevel%==2 set filelist=newbee&&set winver=win11&&echo 你选择了2
if %errorlevel%==3 set filelist=newbee&&set winver=Server2022&&echo 你选择了3
if %errorlevel%==4 set filelist=newbee&&set winver=win8&&echo 你选择了4
if %errorlevel%==8 set filelist=newbee&&set winver=raw&&echo 你选择了8
if %errorlevel%==9 set winver=soft&&echo 你选择了9
...
echo ————————————————————————————
for /f "tokens=2 delims=: " %%a in ('Dism.exe /English /Get-WimInfo /WimFile:"%wimdir%" ^| findstr /i Index') do (
for /f "tokens=2 delims=:" %%b in ('Dism.exe /English /Get-WimInfo /WimFile:"%wimdir%" /Index:%%a ^| findstr /i Name') do (
for /f "tokens=2 delims=:" %%c in ('Dism.exe /English /Get-WimInfo /WimFile:"%wimdir%" /Index:%%a ^| findstr /i Architecture') do (
echo %%a    %%b %%c
)
)
)
...
title 提取WINPE相关文件
echo 提取WINPE相关文件
"%~dp0data\tools\wimlib-imagex.exe" extract "%wimdir%" %inwiminfo% @"%~dp0data\files\install_play_%filelist%_mini.txt" --dest-dir="%~dp0data\winpe" --nullglob --no-acls
