...
-- APP_Path = os.getenv("temp") .. [[\NTBoot]] --ntboot用
-- 提取固定值
...
ck_bios()
App_platform = sui:find("boot_platform") --diskfire用
App_platform.text = "当前环境:" .. bios_str --diskfire用
...
function Add_Boot_Option(addfile)
    ...
    if toadmin == "yes" then
        ...
        exec("/admin", [["%temp%\NTBoot\WinXshell.exe" -script ntboot.lua]])
        return
  ...
  --winapi.show_message('添加文件类型', addfile)
	local filter = "启动镜像|*wim;*vhdx;*vhdx|所有文件|*.*"
		bootfile = Dialog:OpenFile(ntboot_runmode, filter )
...
function check_boot()
    if has_option("-now") or has_option("-r")  then
	  	bootnow = "1" 
	else 
		bootnow = "0" 
	end	
