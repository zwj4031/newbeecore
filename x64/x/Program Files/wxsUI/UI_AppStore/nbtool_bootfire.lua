...
UI.OnClick["item_uefi"] = function()
if boot_platform~= "uefi" then alert("当前环境不支持，需要UEFI环境!") return end
      a=winapi.show_message('警告', "即将重启并进入UEFI固件设置[进入BIOS]", 'yes-no')
	if a == "yes" and File.Exists("X:\\windows\\system32\\shutdown.exe") then
--winapi.show_message('警告', a)
    exec("/wait /hide", [[shutdown -r /fw -t 0 ]])
	elseif a == "yes" and not File.Exists("X:\\windows\\system32\\shutdown.exe") then
--winapi.show_message('警告', a)
    exec(UI_Path .. "\\soft\\shutdown.exe -r /fw -t 0")
else
return
end
    UI:SetTimer("重写列表", 10)
