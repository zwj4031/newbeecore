...
function Add_Boot_Option(addfile)
    ...
    if addfile == "wim" then filter = "启动镜像|*wim|所有文件|*.*"
	bootfile = Dialog:OpenFile("选择你要启动的" .. addfile ..
                                  "文件 当前为:" .. ntboot_runmode, filter)
    elseif addfile == "vhd" then filter = "启动镜像|*vhd;*vhdx|所有文件|*.*"
	bootfile = Dialog:OpenFile("选择你要启动的" .. addfile ..
                                  "文件 当前为:" .. ntboot_runmode, filter)								  
    elseif addfile == "raw" then 
	local filter = "启动镜像|*wim;*vhdx;*vhdx|所有文件|*.*"
	bootfile = Dialog:OpenFile("选择你要启动的文件 当前为:" .. ntboot_runmode, filter)
 	getBootfileExt(bootfile)
	bootsdi(bootfile)
	end
    if bootfile == "" then
