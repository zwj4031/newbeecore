...
function createBCD(boot_platform)
       if boot_platform == "bios" then
   	    ...
   	    exec("/hide /wait", [[cmd /c mkdir ]] .. boot_drv .. [[\\Boot]])
		
	    exec("/show /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\\ms\\bootmgr" ]] .. boot_drv .. [[\]])
		
	   	exec("/hide /wait", [[bcdedit /createstore ]] .. boot_drv .. [[\Boot\BCD.tmp]])
		exec("/hide /wait", [[bcdedit.exe /store ]] .. boot_drv .. [[\\Boot\\BCD.tmp /create {bootmgr} /d "Windows Boot Manager"]])
		exec("/hide /wait", [[bcdedit.exe /import ]] .. boot_drv .. [[\\Boot\\BCD.tmp]])
		exec("/hide /wait", [[bcdedit.exe /set {bootmgr} device partition=]] .. boot_drv)
		end
		if boot_platform == "uefi" then
	    exec("/hide /wait", [[cmd /c mkdir ]] .. boot_drv .. [[\\EFI\\Microsoft\\Boot]])
	    exec("/hide /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\\ms\\bootmgfw.efi" ]] .. boot_drv .. [[\\EFI\\Microsoft\\Boot\\bootmgfw.efi]])
	    exec("/hide /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\\ms\\bootmgfw.efi" ]] .. boot_drv .. [[\\EFI\\Microsoft\\Boot\\bootmgr.efi]])
	    exec("/hide /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\\ms\\bootmgfw.efi" ]] .. boot_drv .. [[\\EFI\\Boot\\bootx64.efi]])
		
	
		exec("/hide /wait", [[bcdedit /createstore ]] .. boot_drv .. [[\EFI\Microsoft\\Boot\\BCD.tmp]])
		exec("/hide /wait", [[bcdedit.exe /store ]] .. boot_drv .. [[\EFI\Microsoft\\Boot\\BCD.tmp /create {bootmgr} /d "Windows Boot Manager"]])
		exec("/hide /wait", [[bcdedit.exe /import ]] .. boot_drv .. [[\EFI\Microsoft\\Boot\\BCD.tmp]])
		exec("/hide /wait", [[bcdedit.exe /set {bootmgr} device partition=]] .. boot_drv)
		end
...
function cs_patition()
	boot_drv = Dialog:BrowseFolder("当前为WinPE环境，但无法找到离线系统，请选择离线系统启动分区", 17)
	if boot_platform == "bios" and File.exists(boot_drv .. [[\Boot\BCD]]) then    
	exec("/hide /wait", [[reg load hklm\BCD_]] .. boot_platform .. [[ ]] .. boot_drv .. [[\Boot\BCD]])
			
	elseif boot_platform == "bios" and not File.exists(boot_drv .. [[\Boot\BCD]]) then
	createBCD(boot_platform)
	exec("/hide /wait", [[reg load hklm\BCD_]] .. boot_platform .. [[ ]] .. boot_drv .. [[\Boot\BCD]])
	
	elseif boot_platform == "uefi" and File.exists(boot_drv .. [[\Boot\BCD]]) then   
	exec("/hide /wait", [[reg load hklm\BCD_]] .. boot_platform .. [[ ]] .. boot_drv .. [[\EFI\Microsoft\Boot\BCD]])	
	
	elseif boot_platform == "uefi" and not File.exists(boot_drv .. [[\EFI\Microsoft\Boot\BCD]]) then   
	createBCD(boot_platform)
		exec("/hide /wait", [[reg load hklm\BCD_]] .. boot_platform .. [[ ]] .. boot_drv .. [[\EFI\Microsoft\Boot\BCD]])	
end
end
...
function boot_bcd_var()
	if boot_platform == "bios" then
		local exitcode, boot_drv_stdout = winapi.execute("cmd /c bcdedit /enum {bootmgr}")
		boot_drv_stdout = boot_drv_stdout:gsub("\r\n", "\n")
		boot_drv = boot_drv_stdout:match("device                  partition=([^\n]+)\n")
		bcd_store = boot_drv .. [[\Boot\BCD]]
	elseif boot_platform == "uefi" then
		local exitcode, boot_drv_stdout = winapi.execute("cmd /c bcdedit /enum {default}")
		boot_drv_stdout = boot_drv_stdout:gsub("\r\n", "\n")
		boot_drv = boot_drv_stdout:match("device                  partition=([^\n]+)\n")
		local exitcode, bcd_drv_stdout = winapi.execute("cmd /c bcdedit /enum {bootmgr}")
		bcd_drv_stdout = bcd_drv_stdout:gsub("\r\n", "\n")
		bcd_drv = bcd_drv_stdout:match("device                  partition=([^\n]+)\n")
		bcd_store = boot_drv .. [[\EFI\Microsoft\Boot\BCD]]
	end
end
...
function ntboot_init()
	if File.exists("X:\\ipxefm\\ipxeboot.txt") or systemdrive == "X:"  then
	 exec("/hide", [[cmd /c mkdir "X:\program files\ms" ]])
	  exec("/hide", [[cmd /c copy /y X:\ipxefm\app\wimboot\*.* "X:\program files\ms"]])
	    get_winver()
		ntboot_addmode = "pe"
		ck_bios()
		check_offline_os()
		ntboot_runmode = "WinPE环境模式![离线系统盘符->" .. boot_drv .. "] [启动环境->" .. bios_str .. "]"
	else
		get_winver()
		boot_drv = os.getenv("systemdrive")
		ck_bios()
		ntboot_addmode = "os"
		ntboot_runmode = "正常系统环境![系统盘符->" .. boot_drv .. "] [启动环境->" .. bios_str .. "]"
	end
end
...
function ntbootnow(bootfile)
	--创建pe目录
	if addfile == "wim" then
		exec("/hide /wait", [[cmd /c mkdir ]] .. boot_drv .. [[\pe]])
		exec("/hide /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\ms\boot.sdi" ]] .. boot_drv .. [[\pe]])
	end
	...
	if ntboot_addmode == "pe" then
		exec("/hide /wait", [[reg unload hklm\BCD_]] .. boot_platform)
			local exitcode, bcdedit_enum = winapi.execute([[bcdedit /enum all|find "]] .. boot_file_path .. [["]])
...
function ntbootadd(bootfile)
	--创建pe目录
	if addfile == "wim" then
		exec("/hide /wait", [[mkdir ]] .. boot_drv .. [[\pe]])
		exec("/hide /wait", [[cmd /c copy /y "]] .. APP_Path .. [[\ms\boot.sdi" ]] .. boot_drv .. [[\pe]])
	end
...
function Add_Boot_Option(addfile)
	...
	bootfile = Dialog:OpenFile("选择你要启动的" .. addfile .. "文件 当前为:" .. ntboot_runmode, filter)
