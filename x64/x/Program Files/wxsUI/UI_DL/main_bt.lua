...
sui:close()
elseif File.exists(app_runpath) and nbapp == "btwim" then 
--winapi.show_message("pid", pid)
exec("/hide", [[cmd /c start "" "X:\Program Files\WinNTSetup\WinNTSetup.exe" nt6 /source:]] .. app_runpath)
sui:close()
end
...
--winapi.show_message("pid", pid)
if app_url ~=nil or user_agent ~=nil and nbapp == "btwim" then
--winapi.show_message("pid", pid)
exec(
    "/hide",
...
function check_app()
	...
	if File.exists(app_runpath) and nbapp == "btwim" then
		winapi.show_message("check_app", "完成")
        label_downspd.text = "下载完毕！请准备安装系统[" .. app_setup .. "]"
        exec("/hide", [[cmd /c start "" "X:\Program Files\WinNTSetup\WinNTSetup.exe" nt6 /source:]] .. app_runpath)
		sui:close()	
...
function check_installed()
     if File.exists(app_runpath) and app_runpath ~= "temp" and nbapp == "btwim" then
	    label_downspd.text = "下载完毕！请准备安装系统[" .. app_setup .. "]"
       	suilib.call("KillTimer", 10088)
		exec("/hide", [[cmd /c start "" "X:\Program Files\WinNTSetup\WinNTSetup.exe" nt6 /source:]] .. app_runpath)
		sui:close()	
